...
trabajos <- read.dbf('datos/CONEVAL_2010/Bases de datos/trabajos.dbf')
...
enigh_raw <- read_csv("datos/enigh_raw.csv")

# verificando faltantes
nas = rep(0, ncol(enigh_raw))
for (i in 1:ncol(enigh_raw)){
  x <- i
  nas[x] = sum(is.na(enigh_raw[,x]))
}
sum(nas)
rm('i', 'x')

base_final <- base_ingreso %>% 
  mutate(hogar_id = str_sub(hogar_id,3,-1)) %>%
  inner_join(enigh_raw, by = 'hogar_id')

# guardando base final
write_csv(x = base_final, path = 'datos/base_final.csv')
