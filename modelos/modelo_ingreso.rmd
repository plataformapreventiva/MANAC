...
ext_fit <- extract(fit)
post_fit <- as.matrix(fit)
# extraer parámetros
betha_mun_raw <- ext_fit$betha_mun_raw
sigma_mun <- ext_fit$sigma_mun
alpha0 <- apply(ext_fit$alpha, 2, mean)
sel_alpha <- grep('alpha', colnames(post_fit))
alpha_pos <- colMeans(post_fit[,sel_alpha])
betha_0 <- ext_fit$betha_0
sel_betha <- grep('betha', colnames(post_fit))
betha_pos <- colMeans(post_fit[,sel_betha])

gen_quantities_r <- function(x_mun, x_hogar){
  betha_mun <- sample(betha_mun_raw, 1) * sample(sigma_mun) + x_mun * alpha_pos
  reg_pred <- mean(betha_0) + x_hogar * betha_pos + betha_mun
  sigma_pred <- sample(sigma_pos)
  log_ing <- normal(reg_pred, sigma_pred)
  return(log_ing)
}
...
for(i in (nrow(x_hogar_entre)+1):nrow(x_hogar)){
  muni <- base_ingreso_hog[i, geo_id]
  pred_log_ing[i] <- gen_quantities_r(x_hogar[i,], x_mun[muni,])
}

log_ingreso_test <- log(1 + base_ingres_hog$ingcor[muestra == 0])
# accuracy
mean(pred_log_ing == log_ingreso_test)
```
